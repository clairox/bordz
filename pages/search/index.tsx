	...
	return (
		<div className={styles['products']}>
			...
			<div className={styles['collection-header']}>
				...
				<h3 className={styles['header-text']}>Search results for {`"${query.q}"`}</h3>
			</div>
...
const FilterDrawer: React.FunctionComponent<FilterDrawerProps> = ({
	...
	}, [maxPrice, minPrice, query, setPriceRange, setSelectedBrands, setSelectedCategories]);
...
export const getServerSideProps: GetServerSideProps = async context => {
	const { page, perPage, priceRange, sort } = context.query as { [key: string]: string };
	let { q } = context.query as { [key: string]: string };
	q = q.split(' ').join(' & ');
